...
CHECK(AuthorisationType IN('teacherUnauthorised', 'teacherAuthorised', 'adminUnauthorised', 'adminAuthorised', 'student')),
CHECK(Gender IN('M', 'F')),
CHECK (Email like('%@%.%')),
PRIMARY KEY(UserID)
);
...
CREATE TABLE Class(
    ...
    CHECK(ClassGroup IN('RED', 'BLUE')),
	CHECK (Level > 0 AND level < 4)      -- Ammend the trigger to add class for subject if values for level modified
...
INSERT INTO User (DateOfBirth, FirstName, LastName, Email, Gender, Password, AuthorisationType)
VALUES ('1970-01-01', 'root', 'admin', 'root@email.com', 'M', 'rootPass123', 'adminAuthorised');
INSERT INTO Administrator (AdminID, DateJoined)
SELECT UserID, CURDATE() FROM User WHERE Email = 'root@email.com';
...
-- Insert students
INSERT INTO User (DateOfBirth, FirstName, LastName, Email, Gender, Password, AuthorisationType)
VALUES ('2005-02-15', 'John', 'Doe', 'john.doe@email.com', 'M', 'studentPass123', 'student'),
       ('2005-05-23', 'Jane', 'Smith', 'jane.smith@email.com', 'F', 'studentPass123', 'student');
INSERT INTO Student (StudentID, Level, ClassGroup)
SELECT UserID, 1, 'RED' FROM User WHERE Email = 'john.doe@email.com';
INSERT INTO Student (StudentID, Level, ClassGroup)
SELECT UserID, 1, 'BLUE' FROM User WHERE Email = 'jane.smith@email.com';
...
INSERT INTO User (DateOfBirth, FirstName, LastName, Email, Gender, Password, AuthorisationType)
VALUES ('1980-03-10', 'Mark', 'Twain', 'mark.twain@email.com', 'M', 'teacherPass123', 'teacherUnauthorised');
INSERT INTO User (DateOfBirth, FirstName, LastName, Email, Gender, Password, AuthorisationType)
VALUES ('1980-03-10', 'Kelvin', 'Lord', 'lord.kelvin@email.com', 'M', 'teacherPass123', 'teacherUnauthorised');
INSERT INTO User (DateOfBirth, FirstName, LastName, Email, Gender, Password, AuthorisationType)
VALUES ('1980-03-10', 'Michael', 'Bron', 'michael.bron@email.com', 'M', 'teacherPass123', 'teacherAuthorised');
INSERT INTO Teacher (TeacherID, SubjectTaught, DateJoined)
SELECT UserID, 'ENG1', CURDATE() FROM User WHERE Email = 'michael.bron@email.com';

-- Assign students to classes
INSERT INTO Class_Student (Level, ClassGroup, SubjectCode, StudentID)
VALUES (1, 'RED', 'MATH1', (SELECT StudentID FROM Student WHERE StudentID = 
                            (SELECT UserID FROM User WHERE Email = 'john.doe@email.com'))),
       (1, 'BLUE', 'ENG1', (SELECT StudentID FROM Student WHERE StudentID = 
                            (SELECT UserID FROM User WHERE Email = 'jane.smith@email.com')));
                           
-- Assign teacher to class level= 1, class group= RED and subject = ENG1 
UPDATE Class SET teacherID = (SELECT teacherID FROM Teacher);

-- Admin approves the unauthorised teacher: Mark Twain
INSERT INTO Approval (AdminID, UserID)
VALUES ((SELECT AdminID FROM Administrator WHERE AdminID = (SELECT UserID FROM User WHERE Email = 'root@email.com')),
        (SELECT UserID FROM User WHERE Email = 'mark.twain@email.com'));
UPDATE User
SET AuthorisationType = 'teacherAuthorised'
WHERE Email = 'mark.twain@email.com';

-- Add a message to the class
INSERT INTO Class_Message(Level, ClassGroup, SubjectCode, Message)
VALUES(1, 'BLUE', 'ENG1', (SELECT TeacherID FROM Class WHERE Level=1 AND ClassGroup='BLUE' AND SubjectCode='ENG1'));